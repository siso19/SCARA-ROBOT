...
# Execution
Action Commander is the integrated execution engine that translates the `cmd_list.json` command file generated by `protocol_to_command_converter.py` into actual robot hardware operations. It sequentially reads JSON commands and coordinates the SCARA robotic arm, DAKEN liquid pump, Servo motor, Gripper, electrochemical measurement system, and JIG fixture in a synchronized order, incorporating safety mechanisms and execution control logic such as surface detection, tip equipping, MEA stack management, pause/stop/resume control, and return-to-home behavior. While `protocol_to_command_converter.py` determines *what should be done*, Action Commander is the layer responsible for *how it is actually performed*.
