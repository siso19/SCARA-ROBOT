...
SCARA-ROBOT is the server architecture for automating Microelectrode Array (MEA) biosensor experiments. It translates declarative experimental protocols designed through a visual interface into sequential robot motion commands, and executes them across a SCARA robotic arm, liquid handling pump, gripper, and electrochemical measurement system. The system supports the fully automated execution of surface functionalization, bacterial sample handling, PBS washing, and CV/EIS measurements without human intervention.

What distinguishes this work is not the automation itself but the abstraction layer that decouples *what the experiment intends* from *how the robot executes*. Researchers express protocols through twelve composable actions — Equip, Eject, Pick, Place, Take, Apply, Mix, Measure, Open, Close, Wait, and Dispose — and the system automatically expands them into hundreds of low-level motion commands while preserving complete execution traceability.
...
**Hardware-Independent Configuration.** All motion parameters and spatial coordinates are externalized to `robot_constants_config.json` and `table.yaml`. This separation allows the same codebase to be applied to different hardware setups or workbench layouts without modifying any code, directly supporting experimental reproducibility.

**MVC-Based Separation of Concerns.** The protocol design layer is structured around the Model-View-Controller pattern, ensuring that protocol state is managed in a single, consistent location. This separation enables independent evolution of data structures, user interfaces, and coordination logic, and makes the system extensible to new experimental workflows.

**End-to-End Automated Workflow.** Surface functionalization, sample handling, washing, and electrochemical measurement (CV/EIS) execute as a single uninterrupted pipeline, producing complete experimental results rather than isolated action demonstrations.

**Accessibility for Domain Scientists.** Biologists without robotics programming experience can design complex automation protocols by visually selecting wells and tubes through the GUI. Internal identifiers (Source IDs) are decoupled from user-defined descriptions (Source Descriptions), allowing meaningful labeling without affecting protocol data stability.

**Complete Execution Traceability.** Every protocol is preserved as a machine-readable command specification (`cmd_list.json`) that records every movement, parameter, and interface call. This intermediate artifact enables post-hoc experiment analysis, debugging, and reproduction by other laboratories — bridging the gap between data science and experimental science.
...
**`mvc_architecture/`** holds the protocol design layer, structured around the Model-View-Controller pattern. The `models/` directory manages the protocol data, the `views/` directory provides the visual interface for selecting targets such as MEA, TIP, SOURCE, and JIG, and the `controllers/` directory coordinates the two and synchronizes protocol state.
