...
**[`MVC_Architecture/`](MVC_Architecture/)** holds the protocol design layer, structured around the Model-View-Controller pattern. The **[`Models/`](MVC_Architecture/)** directory manages the protocol data, the [`Views/`](MVC_Architecture/)** directory provides the visual interface for selecting targets such as MEA, TIP, SOURCE, and JIG, and the [`Controllers/`](MVC_Architecture/)** directory coordinates the two and synchronizes protocol state.
