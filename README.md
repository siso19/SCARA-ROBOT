...
**[`MVC_architecture/`](MVC_architecture/)** holds the protocol design layer, structured around the Model-View-Controller pattern. The **[`models/`](model/)** directory manages the protocol data, the `views/` directory provides the visual interface for selecting targets such as MEA, TIP, SOURCE, and JIG, and the `controllers/` directory coordinates the two and synchronizes protocol state.
